...
const invIndex = inventory.indexOf(33);
console.log(`Car 33 is a ${inventory[32].id} ${inventory[32].car_year} ${inventory[32].car_make} ${inventory[32].car_model}` );
...
//WRONG?? NOT SURE IF I SHOULD MANIPULATE THE STRING.


// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
function carFunction(abc) {
    for (let i = 0; i < inventory.length; i++) {
        abc[i] = inventory[i].car_model;
    }
    carModels.sort();
};
carFunction(carModels);
console.log(carModels);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
function yearsAgg(years) {
    for (let i = 0; i<inventory.length; i++) {
...
const strung = JSON.stringify(BMWAndAudi)
console.log(strung);